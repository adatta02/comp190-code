...
abstract class BaseStatus extends BaseObject  implements Persistent {
  ...
  const PEER = 'StatusPeer';
	...
	public function setState($v)
	{
		...
		if ($this->state !== $v) {
			$this->state = $v;
			$this->modifiedColumns[] = StatusPeer::STATE;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(StatusPeer::STATE)) $criteria->add(StatusPeer::STATE, $this->state);
	...
	public function countProjects(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collProjects === null) {
			if ($this->isNew()) {
				...
				$count = ProjectPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Status has previously been saved, it will retrieve
	 * related Jobs from storage. If this Status is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collJobs === null) {
			if ($this->isNew()) {
			   $this->collJobs = array();
				...
				$criteria->add(JobPeer::STATUS_ID, $this->id);
