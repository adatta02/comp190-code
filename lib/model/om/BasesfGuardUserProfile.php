...
abstract class BasesfGuardUserProfile extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardUserProfilePeer';
	...
	/**
	 * @var        array JobNotes[] Collection to store aggregation of JobNotes objects.
	 */
	protected $collJobNotess;

	/**
	 * @var        Criteria The criteria used to select the current contents of collJobNotess.
	 */
	private $lastJobNotesCriteria = null;
	...
	/**
	 * @var        array Photographer[] Collection to store aggregation of Photographer objects.
	 */
	protected $collPhotographers;

	/**
	 * @var        Criteria The criteria used to select the current contents of collPhotographers.
	 */
	private $lastPhotographerCriteria = null;
	...
	public function setUserTypeId($v)
	{
		...
		if ($this->user_type_id !== $v) {
			$this->user_type_id = $v;
			$this->modifiedColumns[] = sfGuardUserProfilePeer::USER_TYPE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->user_type_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->aUserType = null;
			$this->asfGuardUser = null;
			$this->collJobNotess = null;
			$this->lastJobNotesCriteria = null;

			$this->collJobAttachments = null;
			$this->lastJobAttachmentCriteria = null;

			$this->collPhotographers = null;
			$this->lastPhotographerCriteria = null;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfGuardUserProfilePeer::USER_TYPE_ID)) $criteria->add(sfGuardUserProfilePeer::USER_TYPE_ID, $this->user_type_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getJobNotess() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addJobNotes($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getPhotographers() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addPhotographer($relObj->copy($deepCopy));
				}
			}
	...
	public function getUserType(PropelPDO $con = null)
	{
		if ($this->aUserType === null && ($this->user_type_id !== null)) {
			$c = new Criteria(UserTypePeer::DATABASE_NAME);
			$c->add(UserTypePeer::ID, $this->user_type_id);
			$this->aUserType = UserTypePeer::doSelectOne($c, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aUserType->addsfGuardUserProfiles($this);
			 */
		}
	...
	/**
	 * Clears out the collJobNotess collection (array).
	 *
	 ...
	 * @see        addJobNotess()
	 */
	public function clearJobNotess()
	{
		$this->collJobNotess = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collJobNotess collection (array).
	 *
	 * By default this just sets the collJobNotess collection to an empty array (like clearcollJobNotess());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initJobNotess()
	{
		$this->collJobNotess = array();
	}

	/**
	 * Gets an array of JobNotes objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related JobNotess from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array JobNotes[]
	 * @throws     PropelException
	 */
	public function getJobNotess($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		}
		...
		if ($this->collJobNotess === null) {
			if ($this->isNew()) {
			   $this->collJobNotess = array();
				...
				$criteria->add(JobNotesPeer::USER_ID, $this->id);

				JobNotesPeer::addSelectColumns($criteria);
				$this->collJobNotess = JobNotesPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				JobNotesPeer::addSelectColumns($criteria);
				if (!isset($this->lastJobNotesCriteria) || !$this->lastJobNotesCriteria->equals($criteria)) {
					$this->collJobNotess = JobNotesPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastJobNotesCriteria = $criteria;
		return $this->collJobNotess;
	}

	/**
	 * Returns the number of related JobNotes objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related JobNotes objects.
	 * @throws     PropelException
	 */
	public function countJobNotess(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collJobNotess === null) {
			if ($this->isNew()) {
				...
				$count = JobNotesPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastJobNotesCriteria) || !$this->lastJobNotesCriteria->equals($criteria)) {
					...
					$count = count($this->collJobNotess);
				}
			} else {
				$count = count($this->collJobNotess);
			}
	...
	/**
	 * Method called to associate a JobNotes object to this object
	 * through the JobNotes foreign key attribute.
	 *
	 * @param      JobNotes $l JobNotes
	 * @return     void
	 * @throws     PropelException
	 */
	public function addJobNotes(JobNotes $l)
	{
		if ($this->collJobNotess === null) {
			$this->initJobNotess();
		}
		if (!in_array($l, $this->collJobNotess, true)) { // only add it if the **same** object is not already associated
			array_push($this->collJobNotess, $l);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related JobAttachments from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collJobAttachments === null) {
			if ($this->isNew()) {
			   $this->collJobAttachments = array();
				...
				$criteria->add(JobAttachmentPeer::USER_ID, $this->user_id);
	 ...
	 */
	public function getJobAttachmentsJoinJob($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		}
		...
		if ($this->collJobAttachments === null) {
			if ($this->isNew()) {
				...
				$this->collJobAttachments = JobAttachmentPeer::doSelectJoinJob($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collPhotographers collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addPhotographers()
	 */
	public function clearPhotographers()
	{
		$this->collPhotographers = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collPhotographers collection (array).
	 *
	 * By default this just sets the collPhotographers collection to an empty array (like clearcollPhotographers());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initPhotographers()
	{
		$this->collPhotographers = array();
	}

	/**
	 * Gets an array of Photographer objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related Photographers from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array Photographer[]
	 * @throws     PropelException
	 */
	public function getPhotographers($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserProfilePeer::DATABASE_NAME);
		}
		...
		if ($this->collPhotographers === null) {
			if ($this->isNew()) {
			   $this->collPhotographers = array();
				...
				$this->collPhotographers = PhotographerPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
		...
		$this->lastPhotographerCriteria = $criteria;
		return $this->collPhotographers;
	}
	...
	/**
	 * Method called to associate a Photographer object to this object
	 * through the Photographer foreign key attribute.
	 *
	 * @param      Photographer $l Photographer
	 * @return     void
	 * @throws     PropelException
	 */
	public function addPhotographer(Photographer $l)
	{
		if ($this->collPhotographers === null) {
			$this->initPhotographers();
		}
		if (!in_array($l, $this->collPhotographers, true)) { // only add it if the **same** object is not already associated
			array_push($this->collPhotographers, $l);
			$l->setsfGuardUserProfile($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUserProfile has previously been saved, it will retrieve
	 * related Logs from storage. If this sfGuardUserProfile is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	...
	{
		if ($deep) {
			if ($this->collJobNotess) {
				foreach ((array) $this->collJobNotess as $o) {
			...
			if ($this->collPhotographers) {
				foreach ((array) $this->collPhotographers as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collClients) {
