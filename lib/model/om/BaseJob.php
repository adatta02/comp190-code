...
abstract class BaseJob extends BaseObject  implements Persistent {
  ...
  const PEER = 'JobPeer';
	...
	/**
	 * Set the value of [idr] column.
	 * 
	 * @param      string $v new value
	 * @return     Job The current object (for fluent API support)
	 */
	public function setIdr($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
		...
		if ($this->idr !== $v) {
			$this->idr = $v;
			$this->modifiedColumns[] = JobPeer::IDR;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->end_time = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			...
			$this->zip = ($row[$startcol + 12] !== null) ? (int) $row[$startcol + 12] : null;
			$this->contact_name = ($row[$startcol + 13] !== null) ? (string) $row[$startcol + 13] : null;
			$this->contact_email = ($row[$startcol + 14] !== null) ? (string) $row[$startcol + 14] : null;
			$this->contact_phone = ($row[$startcol + 15] !== null) ? (string) $row[$startcol + 15] : null;
			$this->notes = ($row[$startcol + 16] !== null) ? (string) $row[$startcol + 16] : null;
			$this->estimate = ($row[$startcol + 17] !== null) ? (int) $row[$startcol + 17] : null;
			$this->photo_type = ($row[$startcol + 18] !== null) ? (int) $row[$startcol + 18] : null;
			...
			$this->idr = ($row[$startcol + 23] !== null) ? (string) $row[$startcol + 23] : null;
			$this->resetModified();
			...
			return $startcol + 24; // 24 = JobPeer::NUM_COLUMNS - JobPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[12] => $this->getZip(),
			$keys[13] => $this->getContactName(),
			$keys[14] => $this->getContactEmail(),
			$keys[15] => $this->getContactPhone(),
			$keys[16] => $this->getNotes(),
			$keys[17] => $this->getEstimate(),
			$keys[18] => $this->getPhotoType(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[13], $arr)) $this->setContactName($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setContactEmail($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setContactPhone($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setNotes($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setEstimate($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setPhotoType($arr[$keys[18]]);
		...
		if (array_key_exists($keys[23], $arr)) $this->setIdr($arr[$keys[23]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(JobPeer::IDR)) $criteria->add(JobPeer::IDR, $this->idr);
